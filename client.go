...
// based on Hashi Corp retrtyablehttp with Tor socks5 transport layer
...
var (
	...
	torCheckURL = "https://check.torproject.org"
)
...
func (c *Client) NewRemoteIP() error {
	err := c.Tor.Control.Signal("NEWNYM")
	if err != nil {
		c.Debug("error getting signaling Tor service for new edge IP: %s", err)
		return err
	} else {
		// check that we can connect through tor proxy
		resp, err := c.Get("https://check.torproject.org")
		if err != nil && resp.StatusCode != 200 {
			c.Debug("error connecting to Tor network, trying again...")
...
func (c *Client) Do(req *Request) (*http.Response, error) {
	if c.Logger != nil {
		c.Logger.Infof("[DEBUG] %s %s", req.Method, req.URL)
	}
	...
	for i := 0; ; i++ {
		...
		if err != nil {
			if c.Logger != nil {
				c.Logger.Errorf("[ERR] %s %s request failed: %v", req.Method, req.URL, err)
			}
		} else {
		...
		if c.Logger != nil {
			c.Debug("%s: retrying in %s (%d left)", desc, wait, remain)
		}
		time.Sleep(wait)
	}
